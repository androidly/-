...
async function downloadAllImages() {
     if (imageStates.length === 0) { showStatus("没有图片可下载", 'warning'); return; }
     if (isProcessingGlobal || imageStates.some(s => s.isProcessing)) { showStatus("请等待处理完成", 'warning'); return; }
     isProcessingGlobal = true; showLoading(true); updateButtonStates(); globalStatus.textContent = "下载中...";
     console.log(`开始下载 ${imageStates.length} 张图片...`);
     let successCount = 0, errorCount = 0; const operationStartTime = performance.now();

     for (let i = 0; i < imageStates.length; i++) {
         globalStatus.textContent = `下载 ${i + 1}/${imageStates.length}`;
         const state = imageStates[i];
         if (state && state.currentBlobUrl) {
             try {
                 const link = document.createElement('a');
                 link.href = state.currentBlobUrl;
                 link.download = state.currentFilename || `image_${i + 1}.jpg`; 
                 ...
                 console.error(`下载失败 索引 ${i}`, e);
                 errorCount++;
             }
         } else {
             console.warn(`跳过下载 索引 ${i}: 无有效Blob URL`);
             errorCount++;
         }
     }
     ...
     const operationEndTime = performance.now();
     const totalOperationTime = parseFloat((operationEndTime - operationStartTime).toFixed(2));
     globalStatus.textContent = ""; showLoading(false); isProcessingGlobal = false; updateButtonStates();
     console.log(`下载完成:${successCount}成功,${errorCount}失败(${totalOperationTime}ms)`);
     showStatus(`下载完成 ${successCount}/${imageStates.length}张 (${totalOperationTime}ms)`, errorCount > 0 ? 'warning' : 'success');
 }
...
const modalContents = {
    ...
    about: { title: "关于", content: "<center>小番茄图片批量混淆工具</center><br>版本: 1.1.0 (WASM增强)<br>基于小番茄图片混淆进行深度修改。<br>新增WASM加速，提升处理效率。<br><br><center>贴吧：然而他</center>" }
};
